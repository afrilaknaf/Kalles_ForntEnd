...
export default function Preimum() {
    ...
    const { data, isLoading, isError } = useGets(
        `premium/get`,
        "premium_products"
    )
    ...
    return (
        <>
            <div className="w-full h-auto flex justify-center items-center flex-col mt-5">
                ...
                <p className="w-full md:w-[800px] p-3 text-center text-lg text-[#8c8c8c]">
                    Crafted with precision and timeless elegance,
                    this premium watch reflects the legacy of fine watchmaking.
                </p>
            ...
            <div className="w-full h-auto p-4 flex flex-wrap justify-center items-center gap-5">

                {
                    wacthes.map((item, index) => {
                        ...
                        const isWishlisted = premium?.some((wish) => {
                        ...
                        return (
                            ...
                            <div
                                ...
                                key={item._id}
                                className="w-[280px] md:w-[315px] h-auto p-3 hover:border hover:border-black/30 rounded-lg hover:shadow-lg relative top-0 group"
                            >
                                ...
                                <h1 className="font-bold text-[#C91F28]">
                                    ${item.price}
                                </h1>

                                <button
                                    onClick={() => handlwishlist(item)}
                                    className="w-9 h-9 hidden group-hover:block text-xl rounded-full border absolute top-0 mt-5 ml-2"
                                >
                                ...
                                <button
                                    onClick={() => navigate(`/quick/${item._id}`)}
                                    className="w-9 h-9 text-xl block lg:hidden rounded-full border absolute top-45 end-0 mr-5"
                                >
                                ...
                                <button onClick={() => {
                                    ...
                                    className="w-9 h-9 text-xl block lg:hidden bg-[#C91F28] text-white rounded-full border absolute top-55 mt-3 end-0 mr-5"
                                >
                                ...
                                <div className="w-full h-full flex flex-col gap-3 hidden lg:block">
                                    ...
                                    <button
                                        onClick={() => navigate(`/quick/${item._id}`)}
                                        className="w-full h-[30px] opacity-0 lg:group-hover:opacity-100 transition-all duration-300 border bg-white text-black rounded-2xl font-bold"
                                    >
                                        Quick View
                                    </button>
                                    ...
                                    <button onClick={()=>{
                                        ...
                                        className="w-full h-[30px] opacity-0 lg:group-hover:opacity-100 duration-300 bg-[#C91F28] text-white border rounded-2xl mt-2 font-bold"
                                    >
                                        Quick Add
                                    </button>
            ...
            <div className="flex justify-center items-center m-5">
                ...
                <button
                    onClick={() => navigate("/preimum")}
                    className="text-center w-[150px] h-[40px] bg-black text-white rounded-lg"
                >
                    More Products
                </button>
            </div>
